...
public class ScheduleSwapMoveFactory extends CachedMoveFactory {
	...
	public List<Move> createCachedMoveList(Solution solution) {
		...
		for (ListIterator<Schedule> leftIt = lectureList.listIterator(); leftIt
				.hasNext();) {
			Schedule leftSchedule = leftIt.next();
			for (ListIterator<Schedule> rightIt = lectureList
					.listIterator(leftIt.nextIndex()); rightIt.hasNext();) {
				Schedule rightSchedule = rightIt.next();
				if (!leftSchedule.getCourse().equals(rightSchedule.getCourse())) {
					moveList.add(new ScheduleSwapMove(leftSchedule,
							rightSchedule));
				}
			}
		}
